...
// Enumération des états des robots
enum RobotState {
    Collecting,
    Returning,
}
...
fn draw_map(window: &mut Window, map: &Map, robots: &[Robot]) {
    ...
    for y in 0..map.height {
        for x in 0..map.width {
            ...
            buffer[index] = if map.obstacles[y][x] {
                ...
                0x95a24f // vert olive pour le sol
            };
        }
    }
    ...
    for robot in robots {
        ...
        buffer[index] = match robot.task {
            Task::CollectEnergy => 0xFFFF00FF, // violet pour les robots qui collectent de l'énergie
            ...
            match robot.state {
                RobotState::Collecting => {
                    ...
                    if !map.obstacles[robot.y][robot.x] && !map.water.contains(&(robot.x, robot.y)) {
                        match robot.task {
                            Task::CollectEnergy => {
                                if let Some((index, &(ex, ey))) = map.energy.iter().enumerate().find(|&(_, &(ex, ey))| ex == robot.x && ey == robot.y) {
                                    robot.energy += 1;
                                    map.energy.remove(index);
                            ...
                            Task::CollectMinerals => {
                                if let Some((index, &(mx, my))) = map.minerals.iter().enumerate().find(|&(_, &(mx, my))| mx == robot.x && my == robot.y) {
                                    robot.minerals += 1;
                                    map.minerals.remove(index);
                                    robot.state = RobotState::Returning;
                                } else {
                ...
                RobotState::Returning => {
                    if robot.x == map.base.0 && robot.y == map.base.1 {
                        ...
                        robot.move_towards(map.base, &map);
                    }
                }
            }
        }
